    ...
    public partial class FrmCustomerInfomation : Form
    {
       ...
       List<APP_Data.Customer> customerList = new List<APP_Data.Customer>();
        ...
        private void LoadData()
        {
            ...
            { 
                customerList = (from c in entity.Customers where c.CustomerTypeId == 2 select c).ToList();
            }else if (cboInformationType.Text == "Therapist")
            {
                customerList = (from c in entity.Customers where c.CustomerTypeId == 3 select c).ToList();
            }
            else if (cboInformationType.Text == "Nurse Aid")
            {
                customerList = (from c in entity.Customers where c.CustomerTypeId == 4 select c).ToList();
            }
