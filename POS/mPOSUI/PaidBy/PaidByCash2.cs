            ...
            else
            {
                foreach (Transaction tObj in CreditTransaction)
                {
                    ...
                    {   
                        ...
                        foreach (Transaction Refund in tObj.Transaction1.Where(x=>x.IsDeleted != true && x.Type == "CreditRefund"))
                        {
        ...
        private void btnSubmit_Click(object sender, EventArgs e)
        {
           // string _defaultPrinter = Utility.GetDefaultPrinter();
            ...
            if (!hasError)
            {
                ...
                {
                    long changes = 0;
                    Int64.TryParse(lblChanges.Text, out changes);
                    receiveAmount -= changes;
                    Id = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Sale, true, true, 1, ExtraTax + Tax, ExtraDiscount + Discount, totalCost, receiveAmount, null, CustomerId, MCDiscount, BDDiscount, MemberTypeId, MCDiscountPercent, false, "", IsWholeSale, 0, SettingController.DefaultShop.Id, SettingController.DefaultShop.ShortCode,Note,tableId,servicefee,null,false,cu.Id);
                    ...
                    rv.LocalReport.SetParameters(CustomerName);
                        ReportParameter CusPhoneNumber = new ReportParameter("CusPhoneNumber", customer.PhoneNumber);
                    ...
                    {
                        ReportParameter DiscountAmount = new ReportParameter("DiscountAmount", "-" + (disAmountRep - ExtraDiscount).ToString());
                        rv.LocalReport.SetParameters(DiscountAmount);
                    }
                        if (ExtraDiscount != 0)
                        {
                    ...
                    if (SettingController.TicketSale){                      
                        foreach (var  item in tdlist) {
                            for (int i = 0; i < item.Qty; i++) {
                                System.Data.Objects.ObjectResult<int?> result = entity.InsertTicket(item.Tdid, false, DateTime.Now, item.Category, SettingController.DefaultShop.ShortCode);
                            }
                            System.Data.Objects.ObjectResult<GetTicketBy_TDID_Result> getresult = entity.GetTicketBy_TDID(item.Tdid);

                            var ticketList = getresult.Select(n => new Ticket { TicketNo = n.TicketNo, CreatedDate = n.CreatedDate }).ToList();
                            foreach (Ticket t in ticketList){
                                ...
                                newRow.DateTime = t.CreatedDate.Date.ToString("dd-MM-yyyy");
                                newRow.EventName = shopname;

                                QRCoder.QRCodeGenerator qrgenerator = new QRCoder.QRCodeGenerator();
                                QRCoder.QRCodeData qrdata = qrgenerator.CreateQrCode(t.TicketNo, QRCoder.QRCodeGenerator.ECCLevel.Q);
                                ...
                                rv.LocalReport.ReportPath = reportPath;
                                rv.LocalReport.DataSources.Add(rds);
                                ReportParameter TranNo = new ReportParameter("TranNo", entity.Tickets.Where(a => a.TicketNo==t.TicketNo).FirstOrDefault().TransactionDetail.Transaction.Id);
                ...
                else //isDebt
                {
                    ...
                    if (totalAmount != 0)
                    {
                        ...
                        {
                            CreditTransaction = CreditTransaction.OrderByDescending(x => x.TotalAmount - x.RecieveAmount).ToList();
                    ...
                    if (receiveAmount > 0)
                    {
                        System.Data.Objects.ObjectResult<string> PreDebtId = entity.InsertTransaction(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Prepaid, true, IsPaidByTrans, 1, 0, 0, receiveAmount, receiveAmount, null, CustomerId, MCDiscount, BDDiscount, MemberTypeId, MCDiscountPercent, false, "", IsWholeSale, 0, SettingController.DefaultShop.Id, SettingController.DefaultShop.ShortCode, Note, tableId, servicefee, transactionParentId, false, cu.Id);
                        entity.SaveChanges();
                        ...
                        if (IsPaidByTrans)
                        {
                            ...
                            List<string> CIdList = CreditTransaction.Select(x => x.Id).ToList();
                            var TransId = entity.Transactions.Where(x => x.Type == "Credit" && x.CustomerId == CustomerId && x.IsPaid == false && x.TotalAmount - x.RecieveAmount > 0 && CIdList.Contains(x.Id)).OrderBy(x => x.TotalAmount - x.RecieveAmount).Select(x => x.Id).FirstOrDefault();
                            if (TransId != null)
                            {
                                UsePrePaidDebt usePrePaidDObj = new UsePrePaidDebt();
                                usePrePaidDObj.UseAmount = (int)receiveAmount;
                                usePrePaidDObj.PrePaidDebtTransactionId = resultId;
                                usePrePaidDObj.CreditTransactionId = TransId;
                                usePrePaidDObj.CashierId = MemberShip.UserId;
                                usePrePaidDObj.CounterId = MemberShip.CounterId;
                                entity.UsePrePaidDebts.Add(usePrePaidDObj);
                                entity.SaveChanges();
                    ...
                    ReportParameter TotalAmount = new ReportParameter("TotalAmount", TotalOutstandingAmount.ToString());
                    rv.LocalReport.SetParameters(TotalAmount);
