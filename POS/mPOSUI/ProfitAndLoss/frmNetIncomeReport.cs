...
namespace POS {
    public partial class frmNetIncomeReport : Form
    {
        ...
        private void LoadData()
        {
            ...
            {
                try {
                    var companyYear = Convert.ToDateTime(SettingController.Company_StartDate).Year;
                    var companyMonth = Convert.ToDateTime(SettingController.Company_StartDate).Month;
                    ...
                    #region user filter
                    if ((companyYear < year && companyMonth >= month) || (companyYear == year && companyMonth <= month))
                    {
                        ...
                        else
                        {
                            foreach (var item in data)
                            {
                                ...
                                //finding the product price from the purchase detail
                                PurchaseDetail purchaseDetailEntity = entity.PurchaseDetails.FirstOrDefault(x => x.ProductId == item.Id && x.IsDeleted == false && x.Date.Value.Year == year && x.Date.Value.Month == month);
                                if (purchaseDetailEntity != null) ProductPrice = (int)purchaseDetailEntity.UnitPrice;
                                else
                                {
                                    ...
                                    //  else { Product productEntity = entity.Products.FirstOrDefault(x => x.Id == item.Id); ProductPrice = (int)productEntity.Price; }
                                    else { PurchaseDetail purchasedetail = entity.PurchaseDetails.FirstOrDefault(x => x.ProductId == item.Id); ProductPrice = (int)purchasedetail.UnitPrice; }
                                }
                    ...
                    {
                        MessageBox.Show("No Income data", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                catch(Exception ex)
                {
